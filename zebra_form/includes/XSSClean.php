...
class XSS_Clean
{
	 ...
	 * @access private
	 */
	var $_never_allowed_str = array('document.cookie' => '[removed]', 
			'document.write' => '[removed]', '.parentNode' => '[removed]', 
			'.innerHTML' => '[removed]', 'window.location' => '[removed]', 
			'-moz-binding' => '[removed]', '<!--' => '&lt;!--', 
			'-->' => '--&gt;', '<![CDATA[' => '&lt;![CDATA[', 
			'<comment>' => '&lt;comment&gt;');
	 ...
	 * @access private
	 */
	var $_never_allowed_regex = array('javascript\s*:', 
	...
	/**
	 * Sanitizes submitted data so that Cross Site Scripting Hacks can be
	 * prevented.
	 *
	 * This class is taken from the {@link http://codeigniter.com/ CodeIgniter
	 * PHP Framework},
	 * version 2.1.2.
	 *
	 * <i>This method is automatically run for each control when calling {@link
	 * Zebra_Form::validate() validate()}, unless
	 * specifically disabled by {@link Zebra_Form_Control::disable_xss_filters()
	 * disable_xss_filters()})!</i>
	 *
	 * <b>Following is the original documentation of the class, as found in
	 * CodeIgniter:</b>
	 *
	 * Sanitizes data so that Cross Site Scripting Hacks can be prevented. This
	 * function does a fair amount of work but
	 * it is extremely thorough, designed to prevent even the most obscure XSS
	 * attempts. Nothing is ever 100% foolproof,
	 ...
	 * submission. It's not something that should be used
	 * for general runtime processing.
	 *
	 * This function was based in part on some code and ideas I got from
	 * Bitflux:
	 * {@link http://blog.bitflux.ch/wiki/XSS_Prevention}
	 *
	 * To help develop this script I used this great list of vulnerabilities
	 * along with a few other hacks I've
	 * harvested from examining vulnerabilities in other programs: {@link
	...
	function sanitize ($str)
	{
		...
		$str = preg_replace_callback("/[a-z]+=([\'\"]).*?\\1/si", 
				array($this, '_convert_attribute'), $str);
		$str = preg_replace_callback('/<\w+.*?(?=>|<|$)/si', 
				array($this, '_decode_entity'), $str);
		...
		$words = array('javascript', 'expression', 'vbscript', 'script', 
				'base64', 'applet', 'alert', 'document', 'write', 'cookie', 
				'window');
		...
		foreach ($words as $word)
		{
			...
			$str = preg_replace_callback('#(' . substr($word, 0, - 3) .
					 ')(\W)#is', array($this, '_compact_exploded_words'), $str);
		}
		...
		do
		{
			...
			if (preg_match('/<img/i', $str))
			{
				$str = preg_replace_callback('#<img\s+([^>]*?)(?:\s?/?\>|$)#si', 
						array($this, '_js_img_removal'), $str);
			}
...
return '&lt;' . $matches[1] . $matches[2] . $matches[3] . // encode opening brace
	...
	function _remove_invisible_characters ($str)
	{
		...
		{
			// every control character except newline (dec 10), carriage return
			// (dec 13), and horizontal tab (dec 09),
